...
import { runEvidenceSubstrate } from './evidence-substrate-orchestrator.js';
// =============================================================================
...
// - Populate business_profiles + client_memory
...
const BUSINESS_PROFILE_TABLE = 'business_profiles';
...
const BUSINESS_PROFILE_KEYS = new Set([
  ...
  'evidence_proof', 'evidence_normalized_at', 'needs_review', 'review_items'
]);


export async function previewExistingWebsiteCrawl(input = {}) {
  const startUrl = normalizeUrl(input.url || input.existing_website_url || input.live_url);
  if (!startUrl) throw new Error('Existing website crawl preview requires a valid URL.');
  ...
  const evidence = buildEvidenceOutput({
    crawl,
  ...
  const extracted = {
    ...legacyExtracted,
    normalized_evidence: evidence.normalized,
    evidence_substrate: evidence.substrate,
    business_profile: {
      ...legacyExtracted.business_profile,
      ...evidence.profile_patch,
    },
  };
  ...
  return {
    ...
    pages: crawl.pages.map(p => ({ url: p.url, status: p.status, ok: p.ok })),
    extracted_profile: extracted.business_profile,
    evidence_normalization: evidence.normalized,
    evidence_substrate: evidence.substrate,
    evidence_storage: { persisted: false, reason: 'preview_mode' },
    headings: extracted.headings,
    colors: extracted.colors,
    logo_candidates: extracted.logo_candidates,
...
export async function runExistingWebsiteCrawlAdapter(env, clientRecord, input = {}) {
  ...
  const evidenceStorage = await persistWebsiteIngestion({
    clientDb,
    clientId: clientRecord.id,
    sourceUrl: startUrl,
    crawl,
...
function buildEvidenceOutput({ crawl, extracted, sourceUrl, clientId = null, existingProfile = {} }) {
  const rawArtifacts = {
    ...
    services: extracted.services || [],
    contact: extracted.contact || {},
    contact_methods: contactObjectToMethods(extracted.contact || {}),
    social_links: extracted.social_links || [],
    body_text: crawl.pages.map((p) => p.text || '').join('\n\n').slice(0, 120000),
  };

  const normalized = normalizeCrawlEvidence(rawArtifacts, { source_url: sourceUrl });
  const substrate = runEvidenceSubstrate(rawArtifacts, {
    sourceUrl,
    crawlId: null,
    existingProfile,
  });
  ...
  return {
    ...
    substrate,
    profile_patch: {
      ...toBusinessProfilePatch(normalized),
      needs_review: substrate?.profilePatch?.needs_review || [],
      review_items: substrate?.profilePatch?.review_items || [],
    },
    client_id: clientId,
  };
}

function contactObjectToMethods(contact = {}) {
  const methods = [];
  if (contact.email) methods.push({ type: 'email', value: contact.email, confidence: 0.9, proof: [contact.email] });
  if (contact.phone) methods.push({ type: 'phone', value: contact.phone, confidence: 0.85, proof: [contact.phone] });
  if (contact.hours) methods.push({ type: 'hours', value: contact.hours, confidence: 0.55, proof: [contact.hours] });
  return methods;
}
...
async function persistWebsiteIngestion({ clientDb, clientId, sourceUrl, crawl, extracted, mergePlan, evidence = null }) {
  ...
  if (evidence) {
    return await persistEvidenceSubstrate({ clientDb, clientId, sourceUrl, crawl, extracted, mergePlan, evidence });
  }

  return { persisted: false, reason: 'no_evidence_payload' };
}

async function persistEvidenceSubstrate({ clientDb, clientId, sourceUrl, crawl, extracted, mergePlan, evidence }) {
  const headers = supabaseHeaders(clientDb.key);
  const result = {
    persisted: true,
    crawl_run_id: null,
    evidence_normalization_id: null,
    observations: 0,
    signals: 0,
    review_items: 0,
    errors: [],
  ...
  try {
    const runRows = await supabaseInsertReturning(clientDb, 'crawl_runs', {
      client_id: clientId,
      source_url: sourceUrl,
      status: 'completed',
      pages_crawled: crawl.pages.length,
    ...
    result.crawl_run_id = runRows?.[0]?.id || null;
  } catch (err) {
    result.persisted = false;
    result.errors.push({ table: 'crawl_runs', error: err.message });
  }

  if (result.crawl_run_id) {
    const artifacts = buildCrawlArtifacts(result.crawl_run_id, sourceUrl, crawl, extracted, evidence);
    for (const artifact of artifacts) {
      await supabasePostSafe(clientDb, 'crawl_artifacts', artifact, result.errors);
    }
  }

  try {
    const normRows = await supabaseInsertReturning(clientDb, 'evidence_normalizations', {
      client_id: clientId,
      source_url: sourceUrl,
      source_type: 'website_crawl',
      source_ingestion_id: result.crawl_run_id,
      normalized_json: evidence.normalized || {},
      confidence_json: evidence.normalized?.confidence || {},
      proof_json: evidence.normalized?.proof || {},
      signals_json: evidence.normalized?.signals || [],
      applied_profile_patch: mergePlan.profile_patch || {},
    });
    result.evidence_normalization_id = normRows?.[0]?.id || null;
  } catch (err) {
    result.persisted = false;
    result.errors.push({ table: 'evidence_normalizations', error: err.message });
  }

  const observations = flattenEvidenceObservations(evidence.substrate, result.crawl_run_id, clientId);
  for (const row of observations) {
    await supabasePostSafe(clientDb, 'evidence_observations', row, result.errors);
    result.observations += 1;
  }

  const signalRows = buildSignalRows(evidence.substrate, result.crawl_run_id, clientId);
  for (const row of signalRows) {
    const inserted = await supabaseInsertReturningSafe(clientDb, 'normalized_business_signals', row, result.errors);
    result.signals += 1;
    const signalId = inserted?.[0]?.id || null;
    if (row.should_ask_user) {
      await supabasePostSafe(clientDb, 'signal_review_items', {
        client_id: clientId,
        crawl_run_id: result.crawl_run_id,
        signal_id: signalId,
        field: row.field,
        suggested_value: row.value,
        confidence: row.confidence,
        reason: row.uncertainty_reason || 'Evidence confidence requires user review.',
        clarification_question: clarificationQuestionFor(row.field, row.value),
        status: 'open',
      }, result.errors);
  ...
  for (const [field, value] of Object.entries(mergePlan.profile_patch || {})) {
    await supabasePostSafe(clientDb, 'business_profile_truth_events', {
      client_id: clientId,
      field,
      old_value: null,
      new_value: value,
      action: 'proposed',
      actor: 'system',
      reason: `Proposed from normalized crawl evidence for ${sourceUrl}.`,
    }, result.errors);
  }

  for (const contradiction of mergePlan.contradictions || []) {
    await supabasePostSafe(clientDb, 'business_profile_truth_events', {
      client_id: clientId,
      field: contradiction.key || contradiction.field || 'unknown',
      old_value: contradiction.old_value ?? null,
      new_value: contradiction.website_value ?? contradiction.new_value ?? null,
      action: 'contradicted',
      actor: 'system',
      reason: contradiction.reason || 'Crawl evidence contradicted existing profile memory.',
    }, result.errors);
  }

  if (result.errors.length) result.persisted = false;
  return result;
}

function buildCrawlArtifacts(crawlRunId, sourceUrl, crawl, extracted, evidence) {
  const rows = [];
  for (const page of crawl.pages || []) {
    rows.push({ crawl_run_id: crawlRunId, artifact_type: 'page', source_url: page.url, raw_value: { url: page.url, status: page.status, ok: page.ok, title: extractTitleFromHtml(page.html || '') } });
  }
  for (const heading of extracted.headings || []) rows.push({ crawl_run_id: crawlRunId, artifact_type: 'heading', source_url: sourceUrl, raw_value: heading });
  for (const service of extracted.services || []) rows.push({ crawl_run_id: crawlRunId, artifact_type: 'service_candidate', source_url: sourceUrl, raw_value: service });
  for (const logo of extracted.logo_candidates || []) rows.push({ crawl_run_id: crawlRunId, artifact_type: 'logo_candidate', source_url: sourceUrl, raw_value: logo });
  for (const color of extracted.colors || []) rows.push({ crawl_run_id: crawlRunId, artifact_type: 'color', source_url: sourceUrl, raw_value: color });
  rows.push({ crawl_run_id: crawlRunId, artifact_type: 'normalized_evidence', source_url: sourceUrl, raw_value: evidence.normalized || {} });
  return rows;
}

function flattenEvidenceObservations(substrate, crawlRunId, clientId) {
  const observations = [];
  const proof = substrate?.normalizedSignals?.proof || {};
  for (const items of Object.values(proof)) {
    for (const item of items || []) {
      observations.push({
        ...
        evidence_text: item.text || null,
        extraction_method: item.extractionMethod || item.extraction_method || 'deterministic',
        base_weight: Number(item.baseWeight || item.base_weight || item.confidence || 0.5),
        metadata: item.metadata || {},
      });
    }
  }
  return observations.filter((o) => o.value !== null && o.value !== undefined);
}

function buildSignalRows(substrate, crawlRunId, clientId) {
  const rows = [];
  const signals = substrate?.normalizedSignals || {};
  const add = (field, value, confidence, extra = {}) => {
    ...
    rows.push({
      ...
      uncertainty_reason: extra.uncertainty_reason || null,
      can_autofill: Boolean(extra.can_autofill),
      should_ask_user: Boolean(extra.should_ask_user),
    });
  };

  const industry = signals.industry;
  if (industry) add('industry', industry.value, industry.confidence, {
    evidence_ids: industry.evidence_ids || [],
    candidates: industry.candidates || [],
    uncertainty_reason: industry.uncertainty_reason || null,
    can_autofill: industry.status === 'confident',
    should_ask_user: industry.status !== 'confident',
    status: industry.status || 'candidate',
  });

  for (const svc of signals.primary_services || []) add('primary_services', svc.value, svc.confidence, { status: 'candidate', can_autofill: svc.confidence >= 0.72, should_ask_user: svc.confidence < 0.72 });
  for (const style of signals.visual_style || []) add('visual_style', style.value, style.confidence, { status: 'candidate', can_autofill: style.confidence >= 0.72, should_ask_user: style.confidence < 0.72 });
  for (const color of signals.palette || []) add('visual_palette', color.value || color, color.confidence || 0.65, { status: 'candidate', can_autofill: true, should_ask_user: false });
  return rows;
}

function clarificationQuestionFor(field, value) {
  if (field === 'industry') return `I found evidence that your business may be ${JSON.stringify(value)}. Is that the best category?`;
  if (field === 'primary_services') return `Should ${JSON.stringify(value)} be treated as one of your primary services?`;
  if (field === 'visual_style') return `Should the visual direction include ${JSON.stringify(value)}?`;
  return `Should I use ${JSON.stringify(value)} for ${field}?`;
}

async function supabaseInsertReturning(clientDb, table, row) {
  const res = await fetch(`${clientDb.url}/rest/v1/${table}`, {
    method: 'POST',
    headers: { ...supabaseHeaders(clientDb.key), Prefer: 'return=representation' },
...
async function supabaseInsertReturningSafe(clientDb, table, row, errors) {
  try { return await supabaseInsertReturning(clientDb, table, row); }
  catch (err) { errors.push({ table, error: err.message }); return null; }
}

async function supabasePostSafe(clientDb, table, row, errors) {
  try { await supabasePost(clientDb, table, row); }
  catch (err) { errors.push({ table, error: err.message }); }
}
...
async function fetchCurrentBusinessProfile(clientDb, clientId) {
  const res = await fetch(`${clientDb.url}/rest/v1/${BUSINESS_PROFILE_TABLE}?client_id=eq.${clientId}&select=business_profile&limit=1`, {
...
async function patchClientContext(clientDb, clientId, patch) {
  ...
  await fetch(`${clientDb.url}/rest/v1/${BUSINESS_PROFILE_TABLE}`, {
    method: 'POST',
    headers: {
      ...supabaseHeaders(clientDb.key),

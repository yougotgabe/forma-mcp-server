// functions/api/infrastructure/repair.js
// Returns repair plan or applies repair. Runtime tier uses this directly.
...
export async function onRequest(context) {
  ...
  try {
    const wr = await fetch(`${env.PLATFORM_WORKER_URL}/infrastructure/repair`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json', 'x-worker-secret': env.WORKER_SECRET },
      body:    JSON.stringify({ slug, apply: body.apply === true, check_name: body.check_name }),
    });

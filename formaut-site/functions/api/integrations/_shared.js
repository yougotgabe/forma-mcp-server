// functions/api/integrations/_shared.js
// Shared Cloudflare Pages Function proxy for browser-facing Formaut integration routes.
//
// Browser calls these Pages Function endpoints. The Pages Function verifies the
// signed-in Google user, confirms they are an admin for the client slug, then
// forwards the request server-to-server to the platform Worker with WORKER_SECRET.
//
// Raw API tokens/secrets should only travel through this function and the Worker.

export async function proxyIntegration(context, workerPath) {
  ...
  const slug = body.slug || body.client_id || body.clientId;
  ...
  try {
    const workerRes = await fetch(`${env.PLATFORM_WORKER_URL}${workerPath}`, {
      ...
      body: JSON.stringify({ ...body, slug: clientRecord.slug }),
    });

    const data = await workerRes.json().catch(() => ({}));
    return json(data, workerRes.status);
  } catch {
    return json({ error: 'Could not reach integration service' }, 502);
  }
}

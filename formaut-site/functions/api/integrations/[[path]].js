// functions/api/integrations/[[path]].js
// Cloudflare Pages Function proxy for dashboard integration endpoints.
//
// Browser calls:   /api/integrations/core/connect
// Platform Worker: /integrations/core/connect
//
// This keeps raw client connection secrets off the static site and forwards them
// server-to-server to the Worker, where validation + encryption + storage happen.

export async function onRequest(context) {
  return proxyToPlatformWorker(context, 'integrations');
}
...
async function proxyToPlatformWorker(context, baseSegment) {
  ...
  let upstream;
  try {
    upstream = await fetch(`${workerBase}${workerPath}`, {
      method: 'POST',
      headers,
      body: bodyText,
    });

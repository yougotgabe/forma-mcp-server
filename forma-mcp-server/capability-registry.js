...
export const MCP_TOOL_CAPABILITY_MAP = Object.freeze({
  ...
  bulk_file_read:        null,
  // Agent-optimized composite tools (operator only — no capability gate)
  get_site_health:       null,
  get_business_profile:  null,
  get_pending_reviews:   null,
  stage_content_change:  null,
  commit_staged_change:  null,
  export_agent_package:    null,

  // Autonomous ops tools — operator-gated (null = no capability check, operator key already authorizes)
  preview_staged_change:     null,
  rollback_deployment:       null,
  get_operational_lineage:   null,
  generate_remediation_plan: null,
  ...
  // Email Operations Registry — workspace-scoped MCP tools. Kept ungated by
  // seeded capability rows for now so $10/client-owned projects can use the
  // deterministic registry even before the platform capability table is migrated.
  get_email_workspace:       null,
  register_email_artifact:   null,
  preview_email_artifact:    null,
  register_email_rule:       null,
  activate_email_rule:       null,
  pause_email_rule:          null,
});
